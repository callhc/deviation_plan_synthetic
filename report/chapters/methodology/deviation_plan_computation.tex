...
\emph{BFS depth} of $s$ relative to the \emph{effective detour source}
...
the scenario generator (Section~\ref{par:edge_expanded_network}). Here
...
To handle this, the algorithm walks upstream from the source and downstream
from the target until it reaches junctions with at least two outgoing
(resp.\ incoming) edges. Every intermediate edge encountered during these
walks is added to the blocked set, so the router cannot reuse it as a shortcut.
\textbf{Figure~\ref{fig:detour-endpoints}} illustrates the procedure on a small
graph in which the natural endpoints $s_0$ and $t_0$ are both dead ends,
and the walks promote $s$ and $t$ as the actual detour endpoints.

\begin{algorithm}[htbp]
\caption{\textsc{FindDetourEndpoints} (high-level idea)}\label{alg:find-endpoints}
\begin{algorithmic}[1]
...
As described in \textbf{Algorithm \ref{alg:yen}}, 
...
\begin{algorithm}[htbp]
\caption{\textsc{YenKShortestPaths} (high-level idea)}\label{alg:yen}
\begin{algorithmic}[1]
...
\begin{algorithm}[H]
\caption{\textsc{UpstreamDeviationPlans} (high-level idea)}\label{alg:upstream}
\begin{algorithmic}[1]
...
\textbf{Figure~\ref{fig:candidate-gen}} illustrates the two methods on the same toy
...
\emph{rerouters}\footnote{\url{https://sumo.dlr.de/docs/Simulation/Rerouter.html}}.
...
\texttt{routes\_\{plan\_id\}.xml} file per candidate plan. Only vehicles that
...
$\mathcal{D}^*.P$. This is a splice in the vehicle's edge list, illustrated
in Figure~\ref{fig:route-rewrite}.

\begin{algorithm}[htbp]
\caption{\textsc{DepthAwareRewrite} (high-level idea)}\label{alg:rewrite}
\begin{algorithmic}[1]
