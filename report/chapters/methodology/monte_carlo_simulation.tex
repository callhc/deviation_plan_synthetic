...
\texttt{--weights.random-factor}. With this option, \texttt{duarouter}
perturbs edge costs by drawing multipliers in $[1,\alpha]$. Different
iterations can produce different paths for the same trip endpoints.
...
For each iteration $r$, the raw time-loss difference
\begin{equation}
...
procedure formally, Figure~\ref{fig:mc-workflow} summarizes the workflow.
